...
def normalize_data(data: np.ndarray) -> tuple[np.ndarray, list[MinMaxScaler]]:
    """
    Normalize each feature independently using MinMaxScaler
    
    Args:
        data: Array of shape (N, 5) with features
    
    Returns:
        scaled_data: Normalized data (N, 5)
        scalers: List of 5 scalers (one per feature)
    """
    scalers = []
...
def create_sequences(data: np.ndarray, seq_length: int = 50) -> tuple[np.ndarray, np.ndarray]:
    ...
    for i in range(len(data) - seq_length - 1):
        # Window shape: (seq_length, 5)
        window = data[i:i + seq_length]  # (50, 5)
        # Transpose to (5, 50) - (features, sequence_length)
        window = window.T  # Shape: (5, 50)
        X.append(window)
...
def main() -> tuple[nn.Module, list[MinMaxScaler]]:
    """Main pipeline"""
    ...
    print(f"\n{'='*60}")
    print(f"Stock Prediction CNN - Multi-Feature Training")
    print(f"Features: Open, High, Low, Close, Volume")
    ...
    scaled_data, scalers = normalize_data(features)
    
    # Create sequences with CORRECT shape (batch, 5, seq_length)
